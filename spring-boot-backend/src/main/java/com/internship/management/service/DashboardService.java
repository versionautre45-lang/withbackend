...
public class DashboardService {
    ...
    private DashboardDTO.DashboardMetrics getEncadreurMetrics(Long encadreurId) {
        ...
        long activeProjects = projectRepository.findAll().stream()
                .filter(p -> p.getAssignedInterns().stream().anyMatch(internIds::contains))
                .filter(p -> p.getStatus() == Project.ProjectStatus.IN_PROGRESS)
                .count();
    ...
    private DashboardDTO.DashboardMetrics getStagiaireMetrics(Long userId) {
        ...
        List<Project> myProjects = projectRepository.findByAssignedInternsContaining(intern.getId());
        ...
        double avgProgress = myProjects.isEmpty() ? 0.0 :
                myProjects.stream().mapToInt(Project::getCompletion).average().orElse(0.0);
    ...
    public List<DashboardDTO.DepartmentStats> getDepartmentStats() {
        ...
        for (Project project : projects) {
            for (Long internId : project.getAssignedInterns()) {
                internRepository.findById(internId).ifPresent(intern -> {
                    String dept = intern.getUser().getDepartement();
    ...
    public DashboardDTO.ProjectStatusStats getProjectStatusStats() {
        ...
        long todoCount = projects.stream().filter(p -> p.getStatus() == Project.ProjectStatus.TODO).count();
        long inProgressCount = projects.stream().filter(p -> p.getStatus() == Project.ProjectStatus.IN_PROGRESS).count();
        long doneCount = projects.stream().filter(p -> p.getStatus() == Project.ProjectStatus.DONE).count();
